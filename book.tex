...
Seguire questa facile guida per installare Python \url{https://www.aranzulla.it/come-installare-python-1210886.html},
mettere la spunta sulla casella "Aggiungi Python 3.X al PATH", finita l'installazione, cliccare su "Disable PATH length limit".
Successivamente aprire un terminale in cui digitare \ \ {\tt pip3 install Pygments} \ \ per installare il suddetto modulo
(dopo ogni comando inserito nel terminale, anche nei casi successivi, premere invio per eseguire il comando).
...
In uno dei casi di installazione su Windows il processo non andava a buon fine
nella cartella predefinita, possibili soluzioni:
\begin{itemize}
    \item Eseguire l'installazione con l'antivirus disattivato
    \item Riavviare l'installazione come amministratore
    \item Tornare all'immagine \ref{texlive_config_2} e cambiare la cartella di installazione da {C:/texlive/20XX} a {C:/Programmi/texlive/20XX}.
\end{itemize}
...
Installato Visual Studio Code, seguire la configurazione guidata iniziale
(è possibile saltare questo passaggio), una volta terminata,
...
Terminata l'installazione aprire il menu delle azioni rapide con Ctrl + Shift + P (Cmd + Shift + P su MacOS).
\begin{figure}[H]
    \centering
...
my\_settings.json presente nella cartella del progetto su GitHub
\url{https://github.com/a13ssandr0/TesiUnirc/blob/c610ac6218d6cdb98ae88a5b00fd51c0c3e25d69/my_settings.json}.
\\
\\
\textbf{ATTENZIONE}: Il file che si sta modificando è un file JSON, in quanto
tale richiede alcuni semplici accorgimenti di sintassi.\\
Dopo la prima installazione, il file conterrà esclusivamente due parentesi graffe,
posizionare il cursore tra le due parentesi, premere invio e incollare il contenuto del file sopra menzionato.\\
Se e solo se, come nella figura \ref{json_impostazioni}, dovessero essere già presenti altre 
...
sistema operativo e della configurazione, comparirà una delle tre opzioni sotto, cliccare:
\begin{itemize}
    \item Powershell (Amministratore)
    \item Prompt dei comandi (Amministratore)
    \item Terminale (Admin)
\end{itemize}
Nel prompt scrivere {\tt winget install git.git}, al primo utilizzo winget potrebbe chiedere
di accettare i termini, accettare con {\tt y} e invio quando chiede conferma.

\subsubsection{Installazione su MacOS}
In base alle prove effettuate, risulta già installato su MacOS.

Qualora non fosse installato recarsi sul terminale e scrivere 
\begin{minted}{bash}
   git
\end{minted}
Premere "invio". Il programma installerà automaticamente git dopo aver chiesto conferma per l'installazione degli strumenti sviluppatore.
...
opportunamente modificati con i vostri dati, e premere invio dopo ogni comando.
È molto importante prestare attenzione alle virgolette attorno a nome e email:
\begin{minted}{bash}
...
In caso il menu non dovesse essere come sotto, ma presentasse un errore, nel messaggio
d'errore dovrebbe essere presente un pulsante "reload" per ricaricare VSCode, nel caso
non fosse presente nemmeno quello, chiudere e riaprie VSCode
\begin{figure}[H]
    \centering
...
\chapter {Normale utilizzo}
Durante il normale utilizzo del programma per la scrittura del documento,
è buona norma eseguire un commit delle modifiche ad intervalli regolari, per
sincronizzare il lavoro sul server di GitHub e per avere una cronologia delle modifiche.

I passaggi per eseguire il commit sono:
\begin{enumerate}
    \item Salvare i file modificati
    \item Passare alla scheda "Controllo del codice sorgente" \includegraphics[height=5mm]{images/vscode/controllo_versione_icona_modifiche.png}
    \item Inserire un messaggio descrittivo delle modifiche e seguire cliccando sulla freccetta accanto a "Commit" e poi su "commit e sincronizzazione"
    \begin{figure}[H]
        \centering
        \includegraphics[width=\linewidth]{images/vscode/commit_con_messaggio.png}
        \caption{Commit con messaggio}
        \label{commit_con_messaggio}
    \end{figure}
\end{enumerate}
